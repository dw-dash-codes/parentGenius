...
export default function CourseSolutions() {
  ...
  return (
    <div>
      <TopicHero label={topicName} title="Mini Courses & Solutions" icon={<FaStar size={22} />} />

      <section className="max-w-7xl mx-auto px-6 py-12">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold text-ink-900">Community Solutions for {topicName}</h2>
          <button className="h-10 px-5 rounded-full bg-brand-500 text-white text-sm font-medium transition-colors hover:bg-brand-600 cursor-pointer">
            Upgrade to Unlock
          </button>
        </div>

        {loading ? (
          <p className="text-ink-500 py-6">Loading solutions...</p>
        ) : solutions.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {solutions.map((r) => {
              const solId = r._id || r.id;
              return (
                <div key={solId} className="bg-brand-50 rounded-2xl p-2">
                  <div className="bg-white rounded-xl p-4 ring-1 ring-ink-100 h-full flex flex-col justify-between">
                    <div>
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        />
                        <span className="text-sm font-medium text-ink-900">{r.user || "Parent"}</span>
                      ...
                      <div className="flex items-center gap-1 mb-2">
                        ...
                        <span className="text-xs text-ink-500 ml-1">({r.topic || topicName})</span>
                      </div>
                    ...
                    <button className="text-brand-500 font-medium text-sm hover:underline self-start cursor-pointer">
                      Read More
                    </button>
                  </div>
                </div>
          ...
          <p className="text-ink-500 py-6">No community solutions found for this topic yet.</p>
        )}
      </section>

      <section className="max-w-7xl mx-auto px-6 py-6">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold text-ink-900">Expert Courses</h2>
          <button className="h-10 px-5 rounded-full bg-brand-500 text-white text-sm font-medium transition-colors hover:bg-brand-600 cursor-pointer">
            Upgrade to Unlock
          </button>
        </div>
        ...
        {courses.length > 0 ? (
            ...
            {courses.map((c) => {
              ...
              return (
                <div
                  ...
                  className="relative rounded-2xl overflow-hidden h-64 group cursor-pointer shadow-sm"
                >
                  <img
                  ...
                  <div className="absolute bottom-4 left-4 right-4 text-white">
                    ...
                    <p className="text-xs text-white/80 mb-4 max-w-[85%] line-clamp-2">
                      {c.description || "Teaching practical strategies to build confidence and responsibility..."}
                    </p>
                    <button 
          ...
          <p className="text-ink-500 py-6">No expert courses available for this topic right now.</p>
        )}
      </section>

      <section className="max-w-7xl mx-auto px-6 py-12">
        <h2 className="text-2xl font-bold mb-6 text-ink-900">Resources</h2>
            ...
            >
              ...
              <div className="flex flex-col">
                <h4 className="font-semibold text-sm mb-1 text-ink-900">
                  Parenting Guide & Workbook Vol. {i + 1}
                </h4>
                <p className="text-xs text-ink-500 mb-2">
                  Practical exercises and expert templates for daily family routines...
                </p>
                <p className="text-sm mb-3">
                  <span className="font-bold text-ink-900">$ 27.89</span>{" "}
      ...
      <section className="max-w-7xl mx-auto px-6 py-12">
        <h2 className="text-2xl font-bold mb-6 text-ink-900">1-on-1 Help for Your Family</h2>
        <div className="grid lg:grid-cols-2 gap-10 items-center">
          <div className="relative rounded-2xl overflow-hidden shadow-sm">
            ...
            <div className="absolute bottom-5 left-5 right-5 flex items-end justify-between gap-4">
              <p className="text-white font-semibold text-lg max-w-[70%]">
                Turning everyday tasks into powerful moments of growth and self-worth.
              </p>
              <button 
                onClick={() => navigate("/training/tutorials")}
                className="h-9 px-4 rounded-full bg-brand-500 text-white text-xs font-medium shrink-0 transition-colors hover:bg-brand-600 cursor-pointer"
              >
                Learn More
              </button>
            </div>
          ...
          <div>
            <h3 className="text-xl font-bold text-center mb-6 text-ink-900">
            ...
            <div className="space-y-3 max-w-md mx-auto">
              <button
                ...
                className="w-full h-14 rounded-xl border border-ink-200 text-left px-6 font-medium bg-white text-ink-700 transition-colors hover:border-accent-400 cursor-pointer"
              >
                Yes
              </button>
              <button
